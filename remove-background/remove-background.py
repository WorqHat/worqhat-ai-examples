...
st.title("Remove Image Background using ImageCon API")
st.write("This API enables the removal of the background from images in a single API call, delivering a clear and focused foreground subject without the original backdrop.")
...
if st.button("Send") and uploaded_image is not None:
    ...
    response_v2, duration_v2 = callapi(uploaded_image, api_key)
...
st.markdown(
    "For more information, visit the [Documentation page](https://docs.worqhat.com/ai-models/image-generation/remove-image-background) and the [API Reference page](https://docs.worqhat.com/api-reference/ai-models/image-generation/remove-image-background).")